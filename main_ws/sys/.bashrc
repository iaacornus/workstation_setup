...
PS1="╭╼ \[\e[1;34m\]\h\[\e[0m\] in [\[\e[1;36m\]\w\[\e[0m\]]\n╰─ \`if [ \$? = 0 ]; then echo \[\e[0m\]; else echo \[\e[31m\]; fi\`\u\[\e[0m\]\`if git rev-parse --git-dir > /dev/null 2>&1; then echo '@git:('; fi\`\[\e[1;32m\]\$(parse_git_branch)\[\e[0m\]\`if git rev-parse --git-dir > /dev/null 2>&1; then echo ')'; fi\` ❯ "
